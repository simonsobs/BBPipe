...
class BBMapParamCompSep(PipelineStage):
    ...
    def run(self) :
        ...
        if self.config['Nspec']!=0.0:
            ...
            self.config['number_of_independent_patches'] = 1
        ...
        for i_patch in range(mask_patches.shape[0]):
            ...
            W_ = np.zeros((A_maxL.shape[1], noise_cov__.shape[0], 2, noise_cov__.shape[-1]))
            for p in obs_pix:
                for s in range(2):
                    noise_cov_inv = np.diag(1.0/noise_cov__[:,s,p])
                    inv_AtNA = np.linalg.inv(A_maxL.T.dot(noise_cov_inv).dot(A_maxL))
                    W_[:,:,s,p] = inv_AtNA.dot( A_maxL.T ).dot(noise_cov_inv)
                    noise_after_comp_sep[:,s,p] = W_[:,:,s,p].dot(noise_maps_[:,s,p])
                    if self.config['common_beam_correction']!=0.0:
            ...
            # np.save('W_noise'+str(i_patch), W_)
            # np.save('noise_cov__'+str(i_patch), noise_cov__)
            # np.save('A_maxL'+str(i_patch), A_maxL)
            # np.save('obs_pix'+str(i_patch), obs_pix)
            # np.save('noise_maps_'+str(i_patch), noise_maps_)
            # np.save('noise_after_comp_sep'+str(i_patch), noise_after_comp_sep)
