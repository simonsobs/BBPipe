...
def noise_bias_estimation(self, Cl_func, get_field_func, mask, mask_apo, 
    ...
    else: i_range = range(self.config['Nsims_bias_Nl'])
...
def Cl_stat_res_model_func(self, freq_maps, param_beta,
    ...
    A_ev = A.evaluator(np.array(self.config['frequencies']))
    A_dB_ev = A.diff_evaluator(np.array(self.config['frequencies']))
...
class BBClEstimation(PipelineStage):
    ...
    def run(self):
        ...
        hp.fitsfunc.write_cl(self.get_output('Cl_CMB_template_150GHz'),  np.array(Cl_CMB_template_150GHz), overwrite=True)
